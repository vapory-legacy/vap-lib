...
var Api = function Api(provider) {
  ...
  var addTransactionDefaults = function addTransactionDefaults(tx) {
    return (
      // Get basic defaults
      Promise.all([tx.chainId || send("net_version")(), tx.gasPrice || send("vap_gasPrice")(), tx.nonce || send("vap_getTransactionCount")(tx.from, "latest"), tx.value || "0x0", tx.data || "0x"])
      // Add them to tx
      .then(function (_ref) {
        ...
        if (!tx.gas) {
          ...
          return send("vap_estimateGas")(estimateTx).then(function (usedGas) {
