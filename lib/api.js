...
var Api = function Api(provider) {
  ...
  var sendTransaction = send("vap_sendTransaction");
  var sendRawTransaction = send("vap_sendRawTransaction");
  var getTransactionReceipt = send("vap_getTransactionReceipt");
  var compileSolidity = send("vap_compileSolidity");
  var call = send("vap_call");
  var getBalance = send("vap_getBalance");
  var accounts = send("vap_accounts");
  ...
  var addTransactionDefaults = async function addTransactionDefaults(tx) {
    var baseDefaults = [tx.chainId || send("net_version")(), tx.gasPrice || send("vap_gasPrice")(), tx.nonce || send("vap_getTransactionCount")(tx.from, "latest"), tx.value || "0x0", tx.data || "0x"];
    ...
    var gas = tx.gas ? tx.gas : Nat.div(Nat.mul((await send("vap_estimateGas")({
      from: tx.from,
      to: tx.to,
      value: tx.value,
      nonce: tx.nonce,
