...
const addDefaults = (rpc, tx) => {
  var baseDefaults = [tx.chainId || rpc("net_version", []), tx.gasPrice || rpc("vap_gasPrice", []), tx.nonce || rpc("vap_getTransactionCount", [tx.from, "latest"]), tx.value || "0x0", tx.data || "0x"];
  ...
  return Promise.all(baseDefaults).then(([chainIdNum, gasPrice, nonce, value, data]) => {
    ...
    var gasEstimator = tx.gas ? Promise.resolve(null) : rpc("vap_estimateGas", [{
